...
function generateLinkedInBlogExport() {
  ...
  for (var j = 0; j < collections.length; j++) {
    ...
    var titleKey = "LI_BLOG_" + coll.collection + "_" + coll.category;
    ...
    var gcsUrl = LB_CONFIG.GCS_BASE + "/" + LB_CONFIG.DEFAULT_BATCH + "/Montages/W/" + coll.filename;

    newRows.push([
      article.body,               // Text (full article)
      titleKey,                   // Title (unique key)
      gcsUrl,                     // Media URLs
      coll.category || "General", // Labels
      LB_CONFIG.ACCOUNT_NAME,     // Social Medias (Nmediaservices.com only)
      dateStr                     // Scheduled
    ]);
  }
  ...
  msg += "Mode: " + mode + "\n";
  msg += "Articles created: " + newRows.length + "\n";
