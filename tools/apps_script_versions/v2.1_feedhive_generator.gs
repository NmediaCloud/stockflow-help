...
function readControlPanel_() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (section === "platforms") {
      ...
      // Use platform + account as key to avoid collisions (e.g., TikTok and X both "Stockflow.media")
      var acctKey = platform + "|" + accountName;
      settings.accounts[acctKey] = {
        enabled: enabled,
        limit: dailyLimit,
        platform: platform,
  ...
  for (var fmt of ["W", "S", "V"]) {
    ...
    for (var acctKey in settings.accounts) {
      var acct = settings.accounts[acctKey];
      if (acct.enabled && acct.limit > 0 && acct.formats.indexOf(fmt) !== -1) {
        // Store both the key (for lookup) and the FeedHive account name (for CSV)
        settings.enabledAccounts[fmt].push(acctKey);
      }
    }
  }
...
function generateFeedHiveAll() {
  ...
  for (var j = 0; j < allItems.length; j++) {
    ...
    for (var a = 0; a < enabledAccts.length; a++) {
      var acctKey = enabledAccts[a];
      var acctInfo = settings.accounts[acctKey] || {};
      var platformName = acctInfo.platform || acctKey;
      var feedHiveAcctName = acctInfo.accountName || acctKey.split("|").pop();
      ...
      expandedRows.push({
        content: item,
        acctKey: acctKey,
        accountName: feedHiveAcctName,  // actual FeedHive name for CSV
...
function generateFeedHiveTest() {
  ...
  for (var f = 0; f < formats.length; f++) {
    ...
    // Take first item only — one row PER ACCOUNT
    ...
    for (var a = 0; a < enabledAccts.length; a++) {
      ...
      var platformName = acctInfo.platform || acctKey;
      var feedHiveAcctName = acctInfo.accountName || acctKey.split("|").pop();
      var text = buildPostText_(firstItem, platformName, fmt);
      var titleKey = "TEST_" + (firstItem.collection || firstItem.filename) + "_" + fmt + "_" + platformName;

      var schedDate = now.getFullYear() + "-" +
        String(now.getMonth() + 1).padStart(2, "0") + "-" +
        String(now.getDate()).padStart(2, "0") + " " +
        (testTimes[timeSlot % testTimes.length] || "09:00");
      timeSlot++;
      ...
      testRows.push([
        ...
        feedHiveAcctName,   // actual FeedHive account name
        schedDate
      ]);
    }
  }
...
function gatherAllContent_(formatCode, settings) {
  ...
  // --- 2. Clips from Content Library (synced from Drive — actual filenames) ---
  if (!enabledCT["Clips"]) {
  ...
  try {
    ...
    if (lib) {
      var libData2 = lib.getDataRange().getValues();
      for (var j = 1; j < libData2.length; j++) {
        var cr = libData2[j];
        var cfmt = (cr[LC.FORMAT] || "").toUpperCase();
        if (cfmt !== formatCode) continue;

        var cEnabled = cr[LC.ENABLED];
        var cPublished = cr[LC.PUBLISHED];
        if (cEnabled !== true) continue;
        if (cPublished === true) continue;

        var cType = (cr[LC.CONTENT_TYPE] || "").toLowerCase();
        if (cType !== "clips" && cType !== "clip") continue;

        var cFilename = cr[LC.FILENAME] || "";
        var cCategory = cr[LC.CATEGORY] || "";
        var cSubcategory = cr[LC.SUBCATEGORY] || "";
        var cCollection = cr[LC.COLLECTION] || "";

        // Build GCS URL from actual filename
        var clipGcsUrl = buildGcsUrl_(cFilename, formatCode, "Clips");

        // Get tags from Notes column (enriched during sync)
        var cNotes = cr[LC.NOTES] || "";
        var cTags = "";
        var tagsMatch = cNotes.match(/Tags: (.+)/);
        if (tagsMatch) cTags = tagsMatch[1];
        ...
        allContent.push({
          ...
          filename: cFilename,
          gcsUrl: clipGcsUrl,
          stockflowUrl: cr[LC.STOCKFLOW_URL] || buildStockflowUrl_(cCategory, cSubcategory, cCollection),
          helpUrl: cr[LC.HELP_URL] || "",
          description: "",
          tags: cTags,
          keywords: "",
          priority: 2
...
function fetchClipsData_(formatCode) {
  ...
  for (var i = 1; i < csv.length; i++) {
    ...
    var cleanCat = category.replace(/&/g, "").replace(/  /g, " ").replace(/[ ]/g, "-").replace(/--+/g, "-");
    var cleanSub = subcategory.replace(/[ ]/g, "-").replace(/--+/g, "-");
    var cleanColl = collection.replace(/[ ]/g, "-").replace(/--+/g, "-");
...
function buildPostText_(content, platform, formatCode) {
  ...
  if (platformLower.indexOf("twitter") !== -1 || platformLower.indexOf("x/") !== -1 || platformLower === "x") {
    return buildTwitterText_(name, url, baseTags);
  }
  if (platformLower.indexOf("linkedin") !== -1) {
    return buildLinkedInText_(name, cat, sub, url, desc, baseTags, content.type, isMicro);
  }
  if (platformLower.indexOf("instagram") !== -1) {
    return buildInstagramText_(name, cat, url, desc, baseTags, isMicro);
  }
  if (platformLower.indexOf("tiktok") !== -1) {
    return buildTikTokText_(name, cat, url, baseTags);
  }
  if (platformLower.indexOf("facebook") !== -1) {
    return buildFacebookText_(name, cat, url, desc, baseTags, isMicro);
  }
  ...
  if (content.type === "Montage") {
    ...
    text += "4K video and 8K images. Royalty-free.\n\n";
    if (isMicro) {
...
function buildTwitterText_(name, url, tags) {
  // 280 chars max — short and punchy
  var text = name + " -- Stock footage.\n" + url;
  var tagStr = "\n" + tags.slice(0, 3).join(" ");
  if ((text + tagStr).length <= 280) text += tagStr;
  if (text.length > 280) text = text.substring(0, 277) + "...";
  return text;
}

function buildLinkedInText_(name, cat, sub, url, desc, tags, contentType, isMicro) {
  // 3000 chars max — professional tone
  var text = "New Collection: " + name + "\n\n";
  text += "Premium " + cat.toLowerCase() + " stock footage now available.\n";
  if (sub) text += "Category: " + sub + "\n\n";
  if (desc) text += desc.substring(0, 300) + "\n\n";
  text += "4K Video + 8K Images\n";
  text += "Royalty-free, no attribution\n";
  text += "Instant download\n\n";
  text += "Browse and license: " + url + "\n\n";
  text += tags.slice(0, 5).join(" ");
  if (text.length > 3000) text = text.substring(0, 2997) + "...";
  return text;
}

function buildInstagramText_(name, cat, url, desc, tags, isMicro) {
  // 2200 chars max — hashtag heavy
  var text = name + " -- Premium " + cat.toLowerCase() + " stock footage and images.\n\n";
  if (desc) text += desc.substring(0, 200) + "\n\n";
  text += "4K Video | 8K Images\n";
  text += "Royalty-free | Instant download\n\n";
  text += "Link in bio\n\n";
  text += ".\n.\n.\n";
  text += tags.slice(0, 25).join(" ");
  if (text.length > 2200) text = text.substring(0, 2197) + "...";
  return text;
}

function buildTikTokText_(name, cat, url, tags) {
  // 2200 chars max — short + trending
  var text = name + "\n\n";
  text += "Premium " + cat.toLowerCase() + " stock footage for creators\n";
  text += "Link in bio\n\n";
  text += tags.slice(0, 8).join(" ");
  if (text.length > 2200) text = text.substring(0, 2197) + "...";
  return text;
}

function buildFacebookText_(name, cat, url, desc, tags, isMicro) {
  // 2200 chars max — conversational
  var text = name + "\n\n";
  if (desc) text += desc.substring(0, 300) + "\n\n";
  text += "Premium " + cat.toLowerCase() + " stock footage -- 4K video and 8K images.\n";
  text += "Royalty-free. Instant download.\n\n";
  text += url + "\n\n";
  text += tags.slice(0, 5).join(" ");
  if (text.length > 2200) text = text.substring(0, 2197) + "...";
...
function buildPinterestSchedule_(startDate, totalItems, dailyLimit) {
  var times = FH_CONFIG.PINTEREST.POST_TIMES;
  var limit = dailyLimit || FH_CONFIG.PINTEREST.DAILY_LIMIT || 5;
  // Only use as many time slots as the daily limit allows
  var useTimes = times.slice(0, limit);
  var schedule = [];
  var dayOffset = 0;
  var slotIndex = 0;
  ...
  for (var i = 0; i < totalItems; i++) {
    ...
    var dateStr = d.getFullYear() + "-" +
      String(d.getMonth() + 1).padStart(2, "0") + "-" +
      String(d.getDate()).padStart(2, "0") + "T" + useTimes[slotIndex] + ":00";
    schedule.push(dateStr);
    slotIndex++;
    if (slotIndex >= useTimes.length) {
      slotIndex = 0;
      dayOffset++;
    }
  }
...
function generateFeedHivePerPlatform_(formatCode, tabName, headerColor) {
  ...
  // --- Step 5: Expand to per-account rows ---
  var enabledAccts = settings.enabledAccounts[formatCode] || [];
  var expandedRows = [];

  for (var j = 0; j < allContent.length; j++) {
    var item = allContent[j];
    for (var a = 0; a < enabledAccts.length; a++) {
      var acctKey = enabledAccts[a];
      var acctInfo = settings.accounts[acctKey] || {};
      var platformName = acctInfo.platform || acctKey;
      var feedHiveAcctName = acctInfo.accountName || acctKey.split("|").pop(); // FeedHive account name
      var titleKey = (item.collection || item.filename) + "_" + formatCode + "_" + platformName;
  ...
  // --- Step 6: Schedule per-account ---
  var acctDayOffset = {};
  var acctSlotIndex = {};
  var defaultTimes = FH_CONFIG.POST_TIMES[formatCode] || ["09:00", "13:00"];
  ...
  for (var k = 0; k < expandedRows.length; k++) {
    ...
    var dateKey = d.getFullYear() + "-" +
      String(d.getMonth() + 1).padStart(2, "0") + "-" +
      String(d.getDate()).padStart(2, "0");
    var timeSlot = defaultTimes[slotIdx % defaultTimes.length];

    var text = buildPostText_(er.content, er.platform, formatCode);

    newRows.push([
      text,
      er.titleKey,
      er.content.gcsUrl,
      er.content.category || "General",
      er.accountName,         // ONE account per row
      dateKey + " " + timeSlot
    ]);

    slotIdx++;
    if (slotIdx >= limit) { slotIdx = 0; dayOff++; }
    acctDayOffset[acct] = dayOff;
    acctSlotIndex[acct] = slotIdx;
  }
  ...
  var clipCount = 0;
  for (var m = 0; m < expandedRows.length; m++) {
    if (expandedRows[m].content.type === "Montage") montageCount++;
    else clipCount++;
  }

  // Build friendly account display with daily limits from Control Panel
  var accountDisplay = buildAccountDisplay_(settings);

  // Calculate end date
  var maxDayOff = 0;
  for (var dk in acctDayOffset) {
    if (acctDayOffset[dk] > maxDayOff) maxDayOff = acctDayOffset[dk];
  }
  var endDate = new Date(startDate.getTime() + maxDayOff * 86400000);
  var endDateStr = endDate.getFullYear() + "-" +
    String(endDate.getMonth() + 1).padStart(2, "0") + "-" +
    String(endDate.getDate()).padStart(2, "0");
...
function generatePinterestTab_() {
  ...
  // Get Pinterest daily limit from Control Panel
  var pinterestLimit = FH_CONFIG.PINTEREST.DAILY_LIMIT;
  if (settings.platforms["Pinterest"]) {
    pinterestLimit = settings.platforms["Pinterest"].limit || pinterestLimit;
  }
  var schedule = buildPinterestSchedule_(startDate, newContent.length, pinterestLimit);
  var newRows = [];
